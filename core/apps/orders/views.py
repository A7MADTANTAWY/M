...
def send_notification(user, title, message, notif_type=Notification.IN_APP, order=None):
...
class OrderMarkFinishedView(APIView):
    """Worker marks the job as finished → WAITING_CONFIRMATION."""
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, pk):
        ...
        if order.worker_id != request.user.id:
            return Response(
                {"error": "Only the assigned worker can mark this order as finished."},
                status=status.HTTP_403_FORBIDDEN,
            )

        try:
            order.validate_transition_to(Order.WAITING_CONFIRMATION)
        except DjangoValidationError as exc:
            return validation_error_response(exc)
        ...
        send_notification(
            order.client,
            title="Worker finished the job",
            message=f"{request.user.username} says order #{order.id} is done. Please confirm.",
            notif_type=Notification.PUSH,
        )
        return Response(serialize_order(order, request))
...
class OrderConfirmCompletionView(APIView):
    """Client confirms the job is truly done → COMPLETED."""
    permission_classes = [IsAuthenticated]
    ...
    def post(self, request, pk):
        ...
        if order.client_id != request.user.id:
            return Response(
                {"error": "Only the order's client can confirm completion."},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        profile = WorkerProfile.objects.select_for_update().get(user=order.worker)
        ...
        send_notification(
            order.worker,
            title="Job confirmed ✅",
            message=f"Order #{order.id} has been confirmed as completed by the client.",
            notif_type=Notification.PUSH,
        )
        return Response(serialize_order(order, request))
